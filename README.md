...
| `taptap_probe.py` | TapTap Nuxt payload 解析器（`nuxt_payload` + `deref` + `fetch_platforms`） |
| `fetch_live.py` | 按 app id 抓取详情页，核准评分 / 厂商 / 标签 / 预约数 |
| `validate_recall.py` | 召回率反向验证（用已知 AI 游戏抽查词表命中率） |
| `rescue_scan.py` | 补救扫描：榜单候选 → 评论区特征词 → 信号排序 |
| `seeds.py` | 人工核实的候选种子名单及证据来源 |
| `build_list.py` | 主数据 + 平台映射 + CSV 生成 |
| `build_dashboard.py` | HTML 看板生成 |
| `taptap_live.json` | 最近一次实时抓取的原始结果 |
| `rescue_scan.json` | 最近一次补救扫描的原始结果 |
...
《历史模拟器：崇祯》（app 813198，国内首款 AI 原生历史策略游戏）曾被漏掉。根因是三条召回路径同时失效：

- **标签召回失效**：该游戏 TapTap 标签只有「策略」，无任何 AI 标签
- **关键词召回失效**：简介全文不含 "AI" / "大模型"，用「世界推演模型」表述
- **研报名单召回失效**：4 月走 Steam 首发，未进 ChinaJoy 2026 名单

实测召回率（`validate_recall.py`，5 款已知 AI 游戏）：**标签 40% · 简介 80% · 两者取并 80%**。
漏掉的那一款恰是最重要的崇祯。由此确立三条规则：

1. **平台元数据不可作判定依据**——标签与简介由厂商填写，AI 游戏常主动回避 "AI" 字样（国内舆论下 AI 是负资产）。
2. **关键词词表必须反向验证**——新增词表后用已知 AI 游戏抽查召回率，不能假设"简介里会写"。
3. **证据优先级**：玩家评价 / 第三方深度报道 > 厂商自述 > 平台标签。

`rescue_scan.py` 提供补救路径：抓榜单候选 → 扫评论区特征词（含商业词 `token/积分/自备API`、机制词 `推演/世界模型`、模型名 `千问/DeepSeek`）→ 信号排序。已修正 TapTap 全站页脚（含 TapSight 备案文案）导致的信号虚高问题。

本轮实测 25 款榜单候选未发现新 AI 游戏——**头部榜单的 AI 游戏已收敛，遗漏主要发生在非头部新游池，而 TapTap 不提供该池的公开遍历入口**，需依赖定期媒体监控与社区挖掘。
